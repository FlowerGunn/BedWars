...
public class PhantomListener implements Listener {
    private static final String PHANTOM_PREFIX = "Module:Phantom:";


    @EventHandler
    public void onGolemRegister(BedwarsApplyPropertyToBoughtItem event) {
        if (event.getPropertyName().equalsIgnoreCase("phantom")) {
...
//        Bukkit.getConsoleSender().sendMessage("Interact event");
        ...
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            if (game.getStatus() == GameStatus.RUNNING && !gamePlayer.isSpectator && event.getItem() != null) {
                ...
                String unhidden = APIUtils.unhashFromInvisibleStringStartsWith(stack, PHANTOM_PREFIX);

                if (unhidden != null) {

//                    Bukkit.getConsoleSender().sendMessage("Interact PHANTOM");

                    if (!game.isDelayActive(player, Phantom.class)) {
                        ...
                        Phantom golem = new Phantom(game, player, game.getTeamOfPlayer(player),
...
//                        golem.getEntity().addPotionEffect(new PotionEffect(PotionEffectType.REGENERATION, 100000, 0, false, true));
                        ...
                        int delay = game.getActiveDelay(player, Phantom.class).getRemainDelay();
    ...
    public void onGolemDamage(EntityDamageByEntityEvent event) {
        if (!(event.getEntity() instanceof org.bukkit.entity.Phantom)) {
            return;
        }

        org.bukkit.entity.Phantom ironGolem = (org.bukkit.entity.Phantom) event.getEntity();
        for (String name : Main.getGameNames()) {
            ...
            if (game.getStatus() == GameStatus.RUNNING && ironGolem.getWorld().equals(game.getGameWorld())) {
                List<SpecialItem> phantoms = game.getActivedSpecialItems(Phantom.class);
                for (SpecialItem item : phantoms) {
                    if (item instanceof Phantom) {
                        Phantom golem = (Phantom) item;
    ...
    public void onGolemAttack(EntityDamageByEntityEvent event) {
        ...
        org.bukkit.entity.Phantom ironGolem = (org.bukkit.entity.Phantom) event.getDamager();
        for (String name : Main.getGameNames()) {
            Game game = Main.getGame(name);
            if (game.getStatus() == GameStatus.RUNNING && ironGolem.getWorld().equals(game.getGameWorld())) {
                List<SpecialItem> phantoms = game.getActivedSpecialItems(Phantom.class);
    ...
    public void onGolemTarget(EntityTargetEvent event) {
        ...
        org.bukkit.entity.Phantom entityPhantom = (org.bukkit.entity.Phantom) event.getEntity();
        for (String name : Main.getGameNames()) {
            Game game = Main.getGame(name);
            if ((game.getStatus() == GameStatus.RUNNING || game.getStatus() == GameStatus.GAME_END_CELEBRATING) && entityPhantom.getWorld().equals(game.getGameWorld())) {
                ...
                for (SpecialItem item : activePhantoms) {
                    if (item instanceof Phantom) {
                        ...
                        if (activePhantom.getEntity().equals(entityPhantom)) {
                            if (event.getTarget() instanceof Player) {
                                ...
                                if (Main.isPlayerInGame(player)) {
                                    ...
                                    if (activePhantom.getTeam() == game.getTeamOfPlayer(player)) {
                                        ...
                                        event.setCancelled(true);
//                                        Bukkit.getConsoleSender().sendMessage("Phantom saw an ally and event cancelled");
                                        // Try to find enemy
                                        Player playerTarget = MiscUtils.findTarget(game, player, activePhantom.getFollowRange());
                                        if (playerTarget != null) {
                                            // Oh. We found enemy!
                                            entityPhantom.setTarget(playerTarget);
//                                            Bukkit.getConsoleSender().sendMessage("Phantom from team " + activePhantom.getTeam().getName() + " targets player from team " + game.getTeamOfPlayer(player).getName() + " through event targeting");
                                            return;
                                        }
                                    } else {
//                                        Bukkit.getConsoleSender().sendMessage("Phantom saw an enemy player");
                                    }
                                }
                                ...
                                for (SpecialItem activeSpecialEntity : activePhantoms) {
                                    if (activeSpecialEntity instanceof Phantom) {
                                        ...
                                        if (anotherActivePhantom.getEntity().equals(event.getTarget()) && anotherActivePhantom.getTeam() == activePhantom.getTeam()) {
                                            event.setCancelled(true);
                                        }
                                    }
                                }
    ...
    public void onGolemTargetDie(PlayerDeathEvent event) {
        if (Main.isPlayerInGame(event.getEntity())) {
            ...
            for (SpecialItem item : phantoms) {
                Phantom golem = (Phantom) item;
                org.bukkit.entity.Phantom iron = (org.bukkit.entity.Phantom) golem.getEntity();
    ...
    private String applyProperty(BedwarsApplyPropertyToBoughtItem event) {
        return PHANTOM_PREFIX
                + MiscUtils.getDoubleFromProperty(
                "speed", "specials.phantom.speed", event) + ":"
                + MiscUtils.getDoubleFromProperty(
                "follow-range", "specials.phantom.follow-range", event) + ":"
                + MiscUtils.getDoubleFromProperty(
                "health", "specials.phantom.health", event) + ":"
                + MiscUtils.getBooleanFromProperty(
                "show-name", "specials.phantom.show-name", event) + ":"
                + MiscUtils.getIntFromProperty(
                "delay", "specials.phantom.delay", event) + ":"
                + MiscUtils.getBooleanFromProperty("collidable", "specials.phantom.collidable", event) + ":"
                + MiscUtils.getStringFromProperty(
                "name-format", "specials.phantom.name-format", event);
    }
}
